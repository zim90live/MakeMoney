...
3. ✅ **[维度3] 协方差进 construct 接受判定（已完成 2026-06-08）+ FX 现状厘清**：
   ...
   - ✅ **已补（2026-06-09）FX 折算**：长**回测面板** QDII/黄金（美元计价代理：spx/ixic/gold）现按 **USD/CNY 折人民币口径**。`fetch_usdcny`（`currency_boc_sina` 中行折算价÷100，2005~，种子 `engine/data/idx_usdcny.csv` 5775 行入库·离线可复现）；`build_full_panel(fx_adjust=True)` 把美元总收益级数 ×USD/CNY 水平（按日 ffill 对齐）→ 人民币级数，`fx_adjust=False` 留作"忽略汇率"敏感性对照、种子缺失自动回退。**实测**：人民币 2005(8.28)→2026(6.82) 升值约 21%，标普代理年化 10.6%→**9.7%**、纳指 12.9%→**11.9%**（拖累约 1%/年），A 股不受影响。CLI 全收益长段披露已改口径。测试 +2。**遗留（低优先）**：§0C #1 历史危机标定（`_load_crisis_series`，价格口径）仍未叠加当期 CNY 移动——2008 时 CNY 近乎盯住 6.83、影响小，暂不改。
