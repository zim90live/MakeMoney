...
- 每改一处：跑 `python engine/tests/test_engine.py`（当前 **251 用例**，纯函数、无网络）必须全绿；前端改完 `node --check engine/web/app.js`。
