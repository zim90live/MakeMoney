...
**用户真实标定（live 配置为准，2026-06-08 所有者拍板）**：下表与 `investor_profile.yaml` / strategic 自动算出的 live 值对齐；§2 为权威来源。旧的 100万/70万/8%/20% 已废弃。
...
**核心策略洞察（整个标定的灵魂）**：43.2 万稳健桶是"安全垫"——正因为有它，126.8 万 ETF 桶才能更激进去够 7%，同时把全组合回撤压在 30% 内。
**必须诚实保留**：7% 即便对 ETF 桶也偏进取（回测 ETF 段约 4–6%、长代理段约 8% 但伴 −40%+ 回撤）；且标定显示 **2008 级尾部下 ETF 桶 −38%、按计划满仓折算全组合 −28.5%，逼近 30% 预算**——需权益重仓 + 容忍股票级波动，工具要把权衡量化讲清，绝不暗示稳赚。
...
- `strategy.yaml`：`risk_profile`（回测口径）；`risk_controls`：min_trade 500、max_weekly 50000、first_tranche_pct 0.15、allow_trade_with_cache false。universe 9 只、watchlist 3 只。
...
- **当前长期参数**：总资金 170 万、目标年化 7%、规划年限 30 年、最大回撤约束 30%、失业月开销 6000、失业缓冲 5 年、压力后储备 12 个月 → 自动算出场外稳健桶 43.2 万、本工具计划最大使用金额 126.8 万。
- **当前已应用目标权重（以 `portfolio.yaml` 为准）**：`511010` 25%、`510300` 15%、`510500` 15%、`512890` 5%、`518880` 5%、`513500` 20%、`513100` 5%、`159915` 5%、`588000` 5%。（早先一版 construct 把红利低波/黄金砍到 0%，与"黄金/红利绝不为腾权重而砍"的所有者决策冲突、已废弃。）
...
- **§18 钉死决策（policy_version=2，2026-06-08；权威出处 [STRATEGIC_ALLOCATION_DESIGN.md](STRATEGIC_ALLOCATION_DESIGN.md) §18）**：① 目标收益口径=**ETF 桶**（全组合预期同屏显示、不混用）；② 场外稳健桶=真稳健（存款/货基，2% 收益·0 冲击）；③ 组合级硬上限：非卫星≥70% / 卫星≤20% / 单卫≤10% / 成长≤20% / 单国≤45% / 单风险币≤55%；v2 加黄金/防御 5% 下限 + 解耦构建压力预算 `construct_stress_budget`。
