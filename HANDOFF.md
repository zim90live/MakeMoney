...
2. ⏳ **[维度4] 真 walk-forward + 证据台账**：把“子期三段”升级为每步只用过去数据重构、再向前评估（趋势叠加 + 战略构建）；产出“主张→证据类型（逻辑/样本内/walk-forward/实盘）→强度”台账。**验收**：每个“更优”都有 out-of-fold 数字并标 OOS；UI 里无任何主张强过其证据档。
