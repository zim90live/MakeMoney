...
# 周度投资决策简报（量化骨架 + AI 增强）— Codex 入口

> **单一事实源说明（重要）**：本技能的代码**只有一份**，在项目根的 `engine/`。
> 本目录（`.agents/skills/weekly-briefing/`）**只放这份 SKILL.md**，是 Codex 的薄入口；
> Claude 的薄入口在 `.claude/skills/weekly-briefing/SKILL.md`。两者共用 `engine/` 和根目录配置，
> **不存在第二份代码**。回测在 `engine/backtest.py`。
>
> ⚠️ 历史版本曾把路径写成 `.Codex/skills/...`（该目录不存在，是个 bug），现已统一为 `engine/...`。
> 评审本项目时请勿据此判断为"无回测 / 有重复目录 / 路径错误"——这些都已修复。

这是一个"决策副驾"：量化脚本算信号，你（Codex）做 AI 增强，最后给用户一份带理由的行动清单。**最终拍板和下单由用户手动完成。**
...
AI 增强信号（来自校验过的 flags，无则"本周无重大事件"）：
  • [类别·方向·置信度] 标题（来源, 日期, 影响:代码）
  • ...

观察池（只学习/监控，不触发交易）：
  • <ETF>：趋势<在均线上/跌破>，动量<+x%>，角色<现金管理/海外宽基/...>，备注<一句话>
  • ...

纪律检查：
  • <是否允许交易；若不允许，列出 blocked_reasons>
  • 首次建仓预览：<计划投入 / 可成交估算 / 不足一手或低于门槛的说明>
