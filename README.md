...
**当前状态、不变量与待办**在 [`HANDOFF.md`](HANDOFF.md)（精简·读首屏只需这份）；**变更史与已闭环审计**在 [`HISTORY.md`](HISTORY.md)（只增·极少读）；**设计规范**在两份 `*_ALLOCATION_DESIGN.md`。
当前定位：总资金分「场外稳健垫 + ETF 计划桶」，目标年化只针对 ETF 桶、最大回撤按全组合口径（**具体数值一律以 `investor_profile.yaml` / `portfolio.yaml` 为准，文档不写死**；口径见 HANDOFF.md §1–§2）；P0/P1/P2 已落地（全组合口径风险预算、缓冲感知建议权重、分批建仓回测、目标可行性体检、危机保险提醒等）；长期组合构建已用积木式前瞻收益（债券YTM/A股估值回归/QDII美债+ERP）驱动权重；待办与开放问题见 HANDOFF.md §4。
